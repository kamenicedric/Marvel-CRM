...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { employeeId, lat, lng, mode, selfieDataUrl } = body;
  ...
  // Vérifier qu'il existe déjà un IN aujourd'hui et qu'on n'a pas encore de OUT
  ...
  if (todayErr) {
    return res.status(500).json({ error: todayErr.message });
  }
  ...
  if (!hasIn) {
    return res.status(409).json({ error: "Aucun pointage d'entrée trouvé pour aujourd'hui" });
  }
  if (hasOut) {
    return res.status(409).json({ error: 'Déjà pointé à la sortie aujourd’hui' });
  }
  ...
  if (insErr) {
    return res.status(500).json({
      ...
      details:
        (insErr as { code?: string; hint?: string }).code ||
        (insErr as { code?: string; hint?: string }).hint ||
        '',
    });
  }
