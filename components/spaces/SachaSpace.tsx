...
const SachaSpace: React.FC<SachaSpaceProps> = ({ member }) => {
  const [activeTab, setActiveTab] = useState<'cockpit' | 'planning' | 'performance' | 'teaser'>('cockpit');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Erreur synchro SachaSpace:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const combinedMissions = useMemo(() => {
    ...
    const searchName = "Sacha";

    projects.forEach(p => {
      const teaserData = p.teaser_data as any;
      if (teaserData?.modules?.soiree?.toLowerCase() === searchName.toLowerCase()) {
        missions.push({
          id: `mod-soiree-${p.id}`,
          type: 'module',
          moduleId: 'soiree',
          title: 'MONTAGE PARTIE SOIRÉE',
          couple: p.couple,
          projectId: p.id,
          status: teaserData.module_status?.soiree || 'A faire',
          priority: 'High',
  ...
  if (loading) return (
    <div className="p-20 flex flex-col items-center justify-center h-full gap-4 bg-[#F4F6F3]">
      ...
      <p className="font-black text-[#006344] uppercase italic tracking-[0.3em] animate-pulse">Syncing Matrix Sacha...</p>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
         <div className="bg-[#006344] p-8 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden flex items-center gap-6">
            ...
            <div><p className="text-[9px] font-black text-white/40 uppercase tracking-[0.3em]">Rang Sacha</p><h3 className="text-4xl font-black italic tracking-tighter">#2</h3></div>
         </div>
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-6">
            ...
            <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Rendement</p><h3 className="text-4xl font-black text-[#006344] italic tracking-tighter">92%</h3></div>
         </div>
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-6">
            ...
            <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Primes</p><h3 className="text-3xl font-black text-slate-900 italic tracking-tighter">95.000 F</h3></div>
         </div>
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-6">
            <div className="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center shrink-0">
               <Flame size={32} className="text-orange-500" />
            </div>
            <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Missions</p><h3 className="text-xl font-black uppercase text-slate-900 leading-none">{combinedMissions.filter(m => m.status !== 'Terminé').length} UNITÉS</h3></div>
         </div>
      </div>

      <div className="flex bg-white p-2 rounded-3xl border border-slate-100 shadow-inner w-fit">
         {[{ id: 'cockpit', label: 'Cockpit', icon: LayoutGrid }, { id: 'planning', label: 'Missions Sacha', icon: ListTodo }].map(tab => (
              ...
              {activeMission ? (
                <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 py-10 border-y border-slate-50">
                    <div className="text-center lg:text-left"><p className="text-[9px] font-black text-slate-400 uppercase">Session</p><h4 className="text-4xl font-black text-[#006344] italic uppercase">{isWorking ? 'MONTAGING' : 'READY'}</h4></div>
                    ...
                    <div className="text-center lg:text-right"><p className="text-[9px] font-black text-slate-400 uppercase">Prime Unité</p><h4 className="text-3xl font-black text-[#B6C61A] italic">+ 15.000 F</h4></div>
                  </div>
                  <div className="flex gap-4 mt-12">
                    <button onClick={() => { setIsWorking(!isWorking); handleUpdateStatus(activeMission, 'En cours'); }} className="flex-1 py-6 bg-[#006344] text-[#B6C61A] rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-4">{isWorking ? 'PAUSE' : 'DÉBUTER'}</button>
                    <button onClick={() => handleUpdateStatus(activeMission, 'Terminé')} className="flex-1 py-6 bg-slate-900 text-[#B6C61A] rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-lg flex items-center justify-center gap-4">TERMINER MODULE</button>
                  </div>
                </>
              ) : (
                <div className="py-20 text-center opacity-40"><BadgeCheck size={48} className="mx-auto text-[#B6C61A] mb-4"/><h3 className="text-xl font-black uppercase">Pipeline Sacha libéré</h3></div>
              )}
           </div>
           <div className="xl:col-span-4 space-y-8">
              <div className="bg-slate-900 p-10 rounded-[3.5rem] shadow-2xl text-white relative overflow-hidden">
                 <h4 className="text-2xl font-black uppercase italic tracking-tight mb-8">QUEUE SYSTÈME</h4>
                 <div className="text-5xl font-black italic tracking-tighter text-[#B6C61A]">8 <span className="text-xl text-white opacity-40">JOURS</span></div>
                 <p className="text-[9px] font-bold text-white/40 uppercase tracking-widest mt-6 italic">Expertise : PARTIE SOIRÉE & STORYTELLING</p>
              </div>
           </div>
        </div>
      )}
